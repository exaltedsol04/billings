				  ...
				  <table id="example2" class="table table-striped table-bordered">
					...
					$sqlQuery = $general_cls_call->select_join_query(
						...
						if($sqlQuery[0] != '')
						{
							...
							{	
								...
								SUM(
									CASE
										WHEN u.parent_id = 0
											THEN pv.stock
										WHEN u.conversion > 0
											THEN pv.stock / u.conversion
										ELSE 0
									END
								) AS total_stock_parent_unit
								";
								...
								$totalLooseStock = $general_cls_call->select_query(
									$sql,
									$tables,
									$where,
									$params,
								...
								{
									$measurement_arr = [
										'quantity' => $arr->stock * $arr->measurement,
									...
									//$unitValue = $measurement_units['value'];
									$total_stock = $totalLooseStock->total_stock_parent_unit;
								}
								else {
					...
					?>
                     <tr class="text-center" id="dataRow<?php echo($arr->id);?>">
						...
						<td><?PHP echo (round($arr->stock, 2) == 0) ? 0 : round($arr->stock, 2); ?></td>
						<td><?PHP echo (round($arr->pos_stock, 2) == 0) ? 0 : round($arr->pos_stock, 2); ?></td>
						<td><?PHP echo (round($arr->available_stock, 2) == 0) ? 0 : round($arr->available_stock, 2); ?></td>
