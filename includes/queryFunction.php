<?php
	...
	{ 
		...
		function convert_measurement($measurement_arr)
		{
			$get_data = $this->select_query("*", UNITS, "WHERE id=:id", array(':id'=>$measurement_arr['stock_unit_id']), 1);
			if($get_data->parent_id == 0){
				$unit = [
					'value' => $measurement_arr['quantity'],
					'unit' => $get_data->name,
				];
			}else{
