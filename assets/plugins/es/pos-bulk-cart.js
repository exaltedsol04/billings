let bulkShoppingCart = document.getElementById("bulk-shopping-cart");
let totalAmountShow = document.getElementById("total_amount_show");
/**
 * ! bulkBasket to hold all the selected items
 * ? the getItem part is retrieving bulkData from the local storage
 * ? if local storage is blank, bulkBasket becomes an empty array
 */

let bulkBasket = JSON.parse(localStorage.getItem("bulkData")) || [];
...
function add_to_cart(product) {	
	...
	let qty = 1;
	let search = bulkBasket.find((x) => x.id === selectedItem);
  ...
  update(selectedItem);
  localStorage.setItem("bulkData", JSON.stringify(bulkBasket));
  setTimeout(function () {
	generateBulkCartItems();
  }, 500);
};
...
let calculation = () => {
  ...
  if(cartIcon){
	cartIcon.innerHTML = bulkBasket.map((x) => x.item).reduce((x, y) => x + y, 0);
  }
};
 ...
 * ? When bulkBasket is blank -> show's Cart is Empty
 */

let generateBulkCartItems = () => {
  if (bulkBasket.length !== 0) {
    return (bulkShoppingCart.innerHTML = bulkBasket
      .map((x, index) => {
        ...
        return `<tr id="bulkDataRow${id}">
					<td class="text-center">${index + 1}
					<input type="hidden" class="pid-${pid} allpid" value="${hidMsr}" id="vid_${id}" name="pid[${pid}][${id}]">
					</td>
					...
					<div class="input-group quantity-group">
						...
						<input type="text" class="form-control text-center qty-input${id}" value="${qty}" min="1" oninput="updateQty(${id}, this.value)" id="qty_${id}" name="qty[]">
						<span class="input-group-btn">
				  ...
				  <td class="text-center"><input type="text" name="price[]" class="form-control" value="${price}" oninput="updatePrice(${id}, this.value)" id="price_${id}"></td>						  
    ...
    check_qty_stock(id, search.qty + 1, msr, pid, function () {
        ...
        */
        if (ptype === 'loose' && limit !== null) {
			...
			if (nextWeight > limit) {
                $('#bulkDataRow' + id).find('.qty-increment').prop('disabled', true);
                $("#loader").hide();
                return;
            }
        }
...
let decrement = (id, msr, ptype, pid) => {
    ...
    $('#bulkDataRow' + id).find('.qty-increment').prop('disabled', false);
    ...
    bulkBasket = bulkBasket.filter(x => x.qty !== 0);
    localStorage.setItem("bulkData", JSON.stringify(bulkBasket));
...
let updateQty = (id, value) => {
  ...
  let search = bulkBasket.find((x) => x.id === id);
  if (!search) return;
...
/**
 * ! used to update the selected product item price by value
 */
...
let updatePrice = (id, value) => {
  ...
  search.price = price;
  search.item = price; // keep item count in sync
...
/**
 * ! Used to remove 1 selected product card from bulkBasket
...
let removeItem = (id, ptype, pid) => {
	$("#loader").show();
	let selectedItem = id;
	bulkBasket = bulkBasket.filter((x) => x.id !== selectedItem);
	calculation();
	setTimeout(function () {
		generateBulkCartItems();
	}, 500);
 ...
 * ? When bulkBasket is blank, it will show nothing
 */

let TotalAmount = () => {
  if (bulkBasket.length !== 0) {
    let amount = bulkBasket
      .map((x) => {
