		  ...
		  <div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
			  ...
			  <div class="modal-body">
				<div class="form-body">
					<div class="row g-3">
					<span id="other_mobile_no"></span>
					<div class="col-lg-12 col-md-12 col-sm-12 user_input_div">
							<div class="card-body">
					...
					<div class="d-md-flex d-grid justify-content-md-between">
						<button type="button" class="btn btn-outline-danger px-5" data-bs-dismiss="modal">Cancel</button>
						<button type="button" class="btn btn-grd btn-grd-success px-5 saveUser" onclick="save_users()">save</button>
					 </div>
				  </div>
				  </div>
				</div>
		...
		success: function(response){
			...
			if (result.length > 0) {
				...
				if(inc >= stockCount)
				{
					...
					if(product_type == 'loose' && stockCount == 0) {
						//localStorage.setItem(inputId + '-value', '');
					}
...
function check_non_corp_mobile(val)
{
	...
	$.ajax({
		...
		success: function(response){
			...
			var html = '';
			$.each(result, function (i, user) {
				html += '<span class="text-danger">Mobile is also present in other user, Please try another number!</div>';
				
				$('.user_input_div').hide();
				$('.saveUser').hide();
			});
			$('#other_mobile_no').html(html);
		}
	});
}
...
function save_users()
{
	...
	$.ajax({
		type: "POST",
		url: "<?PHP echo SITE_URL; ?>ajax",
		data: {action: 'savePosUser', pos_user:pos_user, pos_mobile:pos_mobile, user_type:user_type},
		dataType: "json",
...
{
	let rw = 0;
    $('.allpid').each(function () {
        rw += parseFloat($(this).val()) || 0;
    });
	$('#rw').val(rw);
	//cart-list-form
	$('#check-stock-div').html('');
				...
				if (result.length > 0) {
					$.each(result, function (i, stock) {
						html += '<div class="row align-items-start border-bottom py-2">';
							html += '<div class="col-md-6 fw-bold text-break text-nowrap" style="color:#A300A3">' +stock.product_name + '</div>';
							html += '<div class="col-md-2 text-nowrap" style="color:#A300A3">' + stock.variant_name + '</div>';
							html += '<div class="col-md-4 text-danger fw-bold text-end text-nowrap">Available stock ' + stock.variant_stock + '</div>';
						html += '</div>';
					});
					$('#show-products').html(html);
					$('#product-modal').modal('show');
