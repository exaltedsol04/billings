...
//echo $current_weight.'--'.$other_weight.'--'.$remaining_weight.'--'.$remainingStock;die;
				...
				*/
				else
				{
					...
					$total_stock  = $dbStock;
					$checkStock = $dbStock >= $usedQty ? 1 : 0;
				}
//echo $remainingStock; die;
				...
				*/
				...
				$stockArr[] = [
					...
					"variant_stock" => $variant->type == 'loose' ? $remainingStock : $total_stock,
		...
		case "checkStockTransferItem":
			...
			$looseRequested = []; // total requested weight per product
			$looseVariants  = []; // store variant info per product

			foreach($_POST['product_variant_id'] as $k => $variant_id)
			{
				...
				/*
				=========================================
				LOOSE PRODUCT → GROUP BY PRODUCT
				=========================================
				*/
				if($variant->type == 'loose')
				{
				...
				/*
				=========================================
				NORMAL PRODUCT → CHECK PER VARIANT
				=========================================
				*/
				else
				{
					...
					{
						$product = $general_cls_call->select_query(
							"*",
							PRODUCTS,
							"WHERE id = :id",
						...
						$unit = $general_cls_call->select_query(
							"*",
							UNITS,
							"WHERE id = :id",
							[':id'=> $variant->stock_unit_id],
						...
						$stockArr[] = [
							...
							"variant_name"  => $variant->measurement.' '.$unit->name,
			...
			/*
			=================================================
			CHECK LOOSE STOCK ONCE PER PRODUCT
			=================================================
			*/
			foreach($looseRequested as $product_id => $requestedWeight)
			{
				$stock = $general_cls_call->select_query_sum(
					PRODUCT_STOCK_TRANSACTION,
					"WHERE product_id = :product_id
				...
				$totalLooseStock = (float)($stock->total ?? 0);

				if($requestedWeight > $totalLooseStock)
				{
					$variant = $looseVariants[$product_id];
		...
		if(!empty($_POST['product_variant_id']))
		{
			foreach($_POST['product_variant_id'] as $k=>$val) {
				...
				$addExecute=array(
					...
					':product_variant_id'	=> $general_cls_call->specialhtmlremover($val),
					':product_id'			=> $general_cls_call->specialhtmlremover($product_id),
					':loose_stock_quantity'	=> abs($loose_stock_quantity),
					':stock'				=> ($_POST['qty'][$k]),
					':created_date'			=> date("Y-m-d H:i:s"),
					':status'				=> 1,
					':selling_price'		=> $general_cls_call->specialhtmlremover($product_variant_dtls->discounted_price),
...
//echo "<pre>";print_r($qty_used);die;
		...
		case "purchaseAccept":
				...
				$params = [1, $product_id, $product_variant_id];
		...
		case "getProductVariantDeductStock";
				...
				foreach($product_variant_id as $k=>$val)
				{
					...
					elseif($stockType == 2)
					{
						$stockArr[] = [
							...
							"measurement" => $product_variant_dtls->measurement,
							"variant_name" => $p_variant_name,
							"variant_stock" => $stock_used_online->available_stock == null ? 0 : $stock_used_online->available_stock,
						];
					}
				}
				//echo '<pre>'; print_r($stockArr); die;
